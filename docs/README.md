...
- AWS Amplify の利用経験者がチームにおらず、ネット上の開発に関する情報もあまり多くない中で開発を進める必要があった。またアップデートが頻繁に行われており、それに追従する形での対応に追われた。
...
- ドキュメントが不足していたため、情報が不足していると感じた部分はドキュメントを作成しチームに共有することで、チーム全体の業務効率化や生産性向上に貢献した。以下 3 つは具体的な例
  ...
  - デプロイ作業の各種手順
  - amplify の CLI バージョンアップによる影響範囲
- 処理時間の短縮
  - ログイン処理に約 2 秒かかっていたため、処理を見直し約 1 秒まで短縮した
...
- 少人数チームで、小さくリリースしユーザーフィードバックを受け改善するサイクルを高速で回していく開発スタイルを好みます。
...
- 福岡在住のため、他県の場合はフルリモートワークでの勤務を希望します
